...
public class FinanceRatioService {
    ...
    public FinanceRatioService(IncomeRepository incomeRepository, ExpenseRepository expenseRepository, AssetRepository assetRepository, LiabilityRepository liabilityRepository) {
     ...
     */
    public Map<String, RatioResult> calculatedAll(User user) {
    ...
    private RatioResult calculateSavingsRate(BigDecimal income, BigDecimal expenses, String bracket) {
        ...
        double[] benchmark = savingsRateBenchmark(bracket);
        String status = rate.doubleValue() >= benchmarks[1] ? "GOOD" : rate.doubleValue() >= benchmarks[0] ? "WARNING" : "CRITICAL";

        String recommendation = switch (bracket) {
            case "20s" -> "In your 20s, aim to save at least 10-15% of income. Even small amounts compound significantly over tiem.";
            case "30s" -> "In your 30s, target 15-20%. This is a key decade for wealth building - maximize any employer 401k match.";
            case "40s" -> "In your 40s, push for 20-25%. If you're behind on savings, now is the time to accelerate contributions.";
            case "50s" -> "In your 50s, sim for 25-30% and take advantage of catch-up contributions.";
            default -> "At 60+, save as aggressively as possible and review your retirement income plan with an advisor.";
        };

        return new RatioResult("Savings Rate", rate, benchmarks[0], benchmakrs[1], "%", status, bracket, recommendation);
    }
    ...
    private RatioResult calculateDebtToIncome(BigDecimal monthlyDebt, BigDecimal income, String bracket) {
        ...
        String status = ratio.doubleValue() <= benchmark * 0.75 ? "GOOD" : ratio.doubleValue() <= benchmark ? "WARNING" : "CRITICAL";
    ...
    private RatioResult calculateEmergencyFund(BigDecimal liquidAssets, BigDecimal monthlyExpenses, String bracket) {
        ...
        double benchmarks = emrgencyFundBenchmark(bracket);
        String status = ratio.doubleValue() >= benchmarks[1] ? "GOOD" : ratio.doubleValue() >= benchmarks[0] ? "WARNING" : "CRITICAL";
        ...
        return new RatioResult("Emergency Fund", ratio, benchmarks[0], benchmarks[1], " months", status, bracket, recommendation);
    }
     ...
     * target ratio: age / 10
     */
    private RatioResult calculateNetWorth(BigDecimal totalAssets, BigDecimal totalLiabilities, BigDecimal monthlyIncome, int age) {
        ...
        double targe = age / 10.0;
        double good = target;
        double warning = target * 0.5;

        String recommendation = String.format(
            "At age %d, your target net worth is %,1fx your annual income ($%.0f). ", age, target, annualIncome.multiply(BigDecimal.valueOf(target)).doubleValue());
...
private RatioResult calculateDebtToAsset(BigDecimal liabilities, BigDecimal assets, String bracket) {
    ...
    String status = ratio.doubleValue() <= maxGood * 0.6 ? "GOOD" : ratio.doubleValue() <= maxGood ? "WARNING" : "CRITICAL";
...
public static class RatioResult {
    ...
    public RatioResult(String name, BigDecimal value, double benchmarkMin, double benchmarkMax, String unit, String status, String ageBracket, String recommendation) {
        this.name = name;
        this.value = value;
