...
public class BalanceSheetService {
    ...
    public BalanceSheetService(IncomeRepository incomeRepository, ExpenseRepository expenseRepository, SnapshotRepository snapshotRepository) {
...
*/
public MonthkyBalanceSheet getMonthly(Long userId, int year, int month) {
    ...
    BigDecimal savingsRate = totalIncome.compareTo(BigDecimal.ZERO_ == 0 ? BigDecimal.ZERO : netCashFlow.divide(totalIncome, 4, RoundingMode.HALF_UP).multiply(BigDecimal.valueOf(100)).setScale(2, RoundingMode.HALF_UP);
...
public YearlyBalanceSheet getYearly(Long userId, int year) {
    ...
    BigDecimal annualSavingsRate = annualIncome.compareTo(BigDecimal.ZERO) == 0 ? BigDecimal.ZERO : annualNetCashFlow.divide(annualIncome, 4, RoundingMode.HALF_UP).multiply(BigDecimal.valueOf(100)).setScale(2, RoundingMode.HALF_UP);
...
public static class MonthlyBalanceSheet {
    ...
    public MonthlyBalanceSheet(String period, String periodLabel, BigDecimal totalIncome, BigDecimal totalExpenses, BigDecimal netCashFlow, BigDecimal savingsRate, List<CategoryTotal> expensesByCategory, BigDecimal openingNetWorth, BigDecimal closingNetWorth) {
        this.period = period;
...
public static class YearlyBalanceSheet {
    ...
    public YearlyBalanceSheet(int year, List<MonthlyBalanceSheet> months, BigDecimal annualIncome, BigDecimal annualExpenses, BigDecimal annualNetCashFlow, BigDecimal annualSavingsRate, BigDecimal openingNetWorth, BigDecimal closingNetWorth, BigDecimal netWorthChange) {
        this.year = year;
        this.months = months;
