...
import type { SnapshotResponse, RatioResponse, MonthlyBalanceSheet } from '../types';
...
export default function Dashboard() {
  ...
  const nwDelta =
    latest && prev && Number(prev.netWorth) !== 0
      ? ((Number(latest.netWorth) - Number(prev.netWorth)) / Math.abs(Number(prev.netWorth))) * 100
      : null;
    ...
    <div className='p-6 flex flex-col gap-6'>
      ...
      <div className='grid grid-cols-3 gap-4'>
        ...
        <StatCard
          ...
          sub={monthly ? `Savings rate: ${formatPercent(monthly.savingsRate)}` : undefined}
          accent='red'
          icon='↓'
        />
      </div>
      ...
      {snapshots.length === 0 ? (
        <Card className='flex flex-col items-center justify-center py-16 gap-4'>
          ...
          <div className='text-center'>
            ...
            <p className='text-sm text-slate-500 mb-4'>Generate your first snapshot to start tracking net worth</p>
            <Button onClick={generateSnapshot} loading={generating}>Generate First Snapshot</Button>
          </div>
        </Card>
      ) : (
