...
export default function Expenses() {
  ...
  const load = useCallback(async () => {
    setLoading(true);
    try {
      setItems(await expensesApi.getAll(userId, filterCategory || undefined));
    } catch (e: any) {
  ...
  const columns: Column<ExpenseResponse>[] = [
    {
      ...
      width: "120px",
      render: (r) => (
        <span className="text-slate-400 text-sm">{formatDate(r.spentAt)}</span>
      ),
    },
    {
      key: "category",
      ...
      width: "140px",
      render: (r) => (
        <Badge variant={expenseCategoryBadge[r.category] ?? "gray"}>
          {r.category}
        </Badge>
      ),
    },
    ...
    },
    {
      ...
      align: "right",
      render: (r) => (
        <MonoValue value={formatCurrency(r.amount)} className="text-red-600" />
      ),
    },
    {
      key: "actions",
    ...
    <div className="p-6 flex flex-col gap-6">
      ...
      <div
        className={`grid gap-5 ${
          byCategory.length > 0 ? "grid-cols-[1fr_300px]" : "grid-cols-1"
        }`}
      >
        <Card noPad>
          <DataTable
